...
- **画面の高さが十分な端末**（目安: 高さ 700px 以上。実機で調整する）: 入力欄がスクロールで隠れたら、2 行の要約（`○○ の Side U` とタグ、`✎`）を上に固定する。`✎` を押すと一番上に戻り、名前の入力欄にフォーカスする
...
| E2E | 高さ 600px と 850px の画面で、名前・タグのヘッダーの表示が切り替わる | Playwright |
