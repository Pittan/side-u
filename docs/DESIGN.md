...
- 背景は 3D のワイヤーフレーム（三角柱・正四面体・正八面体）がゆっくり回る。Three.js は使わず、Canvas 2D で透視投影して描く（`src/render/wireframe-scene.ts`）。動きを減らす設定のときは止め、画面外では描画しない

- F-1 コンセプトの 1 文と、非公式のファン制作ツールである旨を見せる
- F-2 `Side Uをつくる` で編集画面に入る
- F-3 下書きがあれば、その概要（曲数・先頭の数曲・最終更新）を見せる。完成したときのままなら「完成した Side U を見る」「編集する」、それ以外は「続きから」
...
- F-23 共有画像を 2 つの形で作れる: **9:16（1080×1920）、正方形（1080×1080）**。それぞれ背景を模様あり・透過から選べる
- F-24 画像ごとにできること:
  - **共有する**（Web Share API でファイルを共有できる環境のみ）
  - **保存する**
  - **クリップボードにコピーする**（使える環境のみ。Instagram のストーリーズに貼り付けて写真に重ねる用途）
- F-25 リンクをコピーする
- F-25a 画像の説明文（ALT。`alt` と同じ文章）をコピーする。X などで画像に説明を付けるため
- F-26 編集画面に戻れる
...
形ごとに、背景を **「模様あり」か「透過」か** 選べる（初期値は模様あり）。透過は写真に重ねる用途
...
- 透過のときは、背景を塗らずに PNG の透明度を残す。別の写真の上に置く前提
  - 透過を選んだときだけ、**「文字の後ろを暗くする」をオン・オフできる**（初期値はオン。前回の選択をその端末で覚える）。オンのときは、文字のある左側から右へ、黒から透明になるグラデーションを敷く。背景の写真によって文字が読めなくなるのを防ぐため
  - オフのときも、文字に弱い影を付けて最低限読めるようにする

### 6.2 幾何学模様

- `shared/pattern.ts` で、シード文字列から乱数を作り、図形の配置・角度・色を決める。
- **共有画像の模様は、作るたびに変える**。完成画面を開くたび・「模様を変える」を押すたびに新しいシードを作る。9:16 と正方形は同じシードを使い、背景の「模様あり ／ 透過」の切り替えでは変えない
- **ほかの人の共有ページの背景（SVG）は payload から決める**（同じ URL なら同じ模様）。出力は描画方法に依存しない図形のデータにして、Canvas（共有画像）と SVG（ほかの人の共有ページの背景）の両方で描けるようにする
...
type Draft = {
  ...
  name: string
  updatedAt: string
  completedPayload?: string // 「完成する」を押したときの payload
}
```
